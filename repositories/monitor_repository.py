...
class MonitorRepository:
    def __init__(self, db_path: str = DB_PATH):
        self.db_path=db_path; self._ensure_table(); self._ensure_history_id_column()
    ...
    def _ensure_table(self):
        with self._connect() as conn:
            conn.execute('''CREATE TABLE IF NOT EXISTS monitor_state(
                ...
                symbol TEXT, price REAL, entry_price REAL,
                buy_price_with_fees REAL, pnl REAL,
                updated_at INTEGER)''')
            conn.commit()

    def _ensure_history_id_column(self):
        with self._connect() as conn:
            cols={r[1] for r in conn.execute("PRAGMA table_info(monitor_state)").fetchall()}
    ...
    def save_state(self, token: Token, session: TradeSession):
        ...
        if session.buy_price_with_fees and token.price_native:
            try:
                ...
                pnl=None
        with self._connect() as conn:
            conn.execute('''INSERT INTO monitor_state(
                ...
                ON CONFLICT(pair_address) DO UPDATE SET
                    ...
                    entry_price=excluded.entry_price, buy_price_with_fees=excluded.buy_price_with_fees,
                    pnl=excluded.pnl, updated_at=excluded.updated_at
            ''',(token.pair_address,token.symbol,token.price_native,
                 session.entry_price,session.buy_price_with_fees,pnl,token.pair_address))
            conn.commit()

    @log_function
    def set_history_id(self, pair_address: str, history_id: int):
        with self._connect() as conn:
            conn.execute('''INSERT INTO monitor_state(pair_address,history_id,updated_at)
                            VALUES(?, ?, strftime('%s','now'))
    ...
    @log_function
    def clear_history_id(self, pair_address:str):
        with self._connect() as conn:
    ...
    def list_monitored(self, limit:int=50)->list[dict]:
        with self._connect() as conn:
            cur=conn.execute("""SELECT pair_address,symbol,price,entry_price,
                                buy_price_with_fees,pnl,updated_at,history_id
                                FROM monitor_state ORDER BY updated_at DESC LIMIT ?""",(limit,))
